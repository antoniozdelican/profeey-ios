...
//        AWSClientManager.defaultClientManager().userPool?.currentUser()?.getDetails().continue({
//            (task: AWSTask) in
...
//                return AWSTask(result: result).continue(completionHandler)
//            } else {
//                print("This should not happen with getUserDetailsUserPool.")
//                return AWSTask(result: nil).continue(completionHandler)
//            }
//        })
    }
